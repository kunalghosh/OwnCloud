...
if (empty($user)) {
    ...
    ##/search_session()
    if (!empty($user))
        lg("for session $sessid found user $user.");
    if (empty($user))
        $user = "anonymous";
...
// Also if the user exists in the owncloud's database!
    /* TODO:
     * 
     * If the user attemps to login, and it exists on the OC database (checked above)
     * we should create an entry for him in the app table here and all the other neccesary stuff
     * 
     * The entry on the apptable should have all the neccesary info of the profile file.
     */
...
$authenticated = (bool) (
        ...
        or (($pass == user_profile_get("password")) and (OC_USER::checkPassword($user, $pass) ))
        );
if ($authenticated) {
...
foreach ($syncbod->children() as $key => $value) {
    ...
    if (empty($lcli))
        $lcli = $value->Item->Source;
    if (empty($lsrv))
        $lsrv = $value->Item->Target;
    if (empty($lcli))
        $lcli = $value->Source->LocURI;
    if (empty($lsrv))
        $lsrv = $value->Target->LocURI;
    if (empty($lcli))
        $lcli = $value->Source;
    if (empty($lsrv))
        $lsrv = $value->Target;
    ...
    switch ($key) {
        ...
        case "Sync":
            ...
            $i++;
            $send .= $sync_re;
            if (isset($value->NumberOfChanges))
                lg("NumberOfChanges: " . $value->NumberOfChanges);
                ...
                if (in_array($skey, array("Add", "Replace", "Delete"))) {
                    foreach ($svalue->children() as $sskey => $ssvalue) {
                        if ($sskey == "Item") {
                            ...
                            if (empty($slcli))
                                $slcli = $ssvalue->Source;#SyncLocClient
                            if (empty($slsrv))
                                $slsrv = $ssvalue->Target;#SyncLocServer
                            ...
                            if (!empty($slcli))
                                $slcli_array = explode("/", $slcli);
                            if (empty($slsrv) && !empty($slcli))
                                $slsrv = $lsrv . "/" . end($slcli_array);
                            #lg("NEW SLSRV $slsrv");
